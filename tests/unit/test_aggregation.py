...
def test_compute_window_boundaries_hour() -> None:
    ref = datetime(2026, 6, 30, 14, 35, 0, tzinfo=UTC)
    ws, we = agg_service.compute_window_boundaries("hour", ref)
    assert ws == datetime(2026, 6, 30, 13, 0, 0, tzinfo=UTC)
    assert we == datetime(2026, 6, 30, 14, 0, 0, tzinfo=UTC)


def test_compute_window_boundaries_day() -> None:
    ref = datetime(2026, 6, 30, 14, 35, 0, tzinfo=UTC)
    ws, we = agg_service.compute_window_boundaries("day", ref)
    assert ws == datetime(2026, 6, 29, 0, 0, 0, tzinfo=UTC)
    assert we == datetime(2026, 6, 30, 0, 0, 0, tzinfo=UTC)
    ...
    for i in range(10):
        await event_service.ingest_view_event(
            ...
            ViewEventRequest(
                ...
                viewer_id=f"u{i}",
                event_time=datetime(2026, 6, 30, 13, 30, 0, tzinfo=UTC),
            ),
        )
    for i in range(5):
        await event_service.ingest_view_event(
            ...
            ViewEventRequest(
                ...
                viewer_id=f"u{i+100}",
                event_time=datetime(2026, 6, 30, 13, 45, 0, tzinfo=UTC),
            ),
        )
    await db_session.flush()
...
async def test_empty_window_returns_empty(db_session: AsyncSession) -> None:
    ws = datetime(2026, 6, 30, 10, 0, 0, tzinfo=UTC)
    we = datetime(2026, 6, 30, 11, 0, 0, tzinfo=UTC)
    results = await agg_service.get_top_k(db_session, ws, we, 10)
    assert results == []
...
async def test_upsert_rerun_idempotent(db_session: AsyncSession) -> None:
    ...
    await event_service.ingest_view_event(
        ...
        ViewEventRequest(
            ...
            video_id=v.video_id,
            viewer_id="u1",
            event_time=datetime(2026, 6, 30, 14, 30, 0, tzinfo=UTC),
        ),
    )
    await db_session.flush()
